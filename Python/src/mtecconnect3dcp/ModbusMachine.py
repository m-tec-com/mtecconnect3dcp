...
class ModbusMachine:
    ...
    """
    def __init__(self, port: Optional[str] = None, frequency_inverter_id: str = "01", baudrate: int = 19200, log: bool = False):
        self.port = port
        self.frequency_inverter_id = frequency_inverter_id
    ...
    def connect(self, port: Optional[str] = None):
        ...
        Args:
            port (str): Serial port to use. If not provided, uses the one from init.
        """
        if port:
            self.port = port
        if not self.port:
    ...
    def _send_command(self, parameter: str, value: int) -> Any:
        data = self.frequency_inverter_id + parameter + self._int2hex(value, 4)
    ...
    def _wait_for_response(self) -> Any:
        ...
        while True:
            if self.serial.inWaiting() >= complete_data_length:
                break
            if time.time_ns() > timeout:
        ...
        if message_type == 3:
            ...
            for _ in range(message_length):
                message_value = (message_value << 8) + int.from_bytes(self.serial.read(1), "little")
                command += self._int2hex(message_value & 0xFF, 2)
        elif message_type == 6:
            param = self._int2hex(int.from_bytes(self.serial.read(1), "little"), 2) + self._int2hex(int.from_bytes(self.serial.read(1), "little"), 2)
            command += param
            value0 = int.from_bytes(self.serial.read(1), "little")
            value1 = int.from_bytes(self.serial.read(1), "little")
        ...
        crc = self._int2hex(int.from_bytes(self.serial.read(1), "little"), 2) + self._int2hex(int.from_bytes(self.serial.read(1), "little"), 2)
    ...
    def _keepalive_loop(self):
        if not self.connected:
            return
        value = self._send_hex_command(self.frequency_inverter_id + self._keepalive_command)
